  ...
  def __call__(self, input):
    ...
    with tf.variable_scope(self.name):
      ...
      with tf.variable_scope('first', reuse=self.reuse):
          ...
          hidden_output = tf.matmul(input, weights1) + biases1

      # hidden layer --> Y
      with tf.variable_scope('second', reuse=self.reuse):
          ...
          biases2 = tf.get_variable('biases2',
                                   ...
                                   dtype=tf.float32,
                                   initializer=tf.constant_initializer(0.1))
          output = tf.matmul(hidden_output, weights2) + biases2
